...
auth.post('/register', async (req, res) => {
    ...
    const userExists = User.findOne({ email: req.body.email }, (err, response) => {
        if (err) {
            ...
            if (response) {
                res.status(300).send("The email already exists")
            }
            else {
                const user = new User({
                    name: req.body.name,
...
auth.post("/login", (req, res) => {
    ...
    const userLogin = User.findOne({ email: req.body.email }, (err, response) => {
        if (err) {
            ...
            if (!response) {
                res.status(300).send("There is no account with this email!");
                ...
                if (req.body.password == response.password) {
                    ...
                    res.status(300).send("You have entered the wrong password.")
                }
            }
        }
    })
